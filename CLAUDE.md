...
- `server.py` — the merged MCP `Server`. Every handler re-reads the request policy via `get_policy()`. **Admin lockdown:** `list_tools()` returns `[]` for non-admin keys — clients are expected to discover tools via `/tool-rag/retrieve` instead. `call_tool` still works for any allowed tool.
