...
- `docker` — emits a service (with a `build:` context) into `docker-compose.servers.yml`, registers a `streamable_http`/`sse` URL (service name in compose, or `127.0.0.1:<port>` with `--host`). The image is built by `docker compose up --build`, not by the provisioner — so provisioning can itself run in a container with no Docker socket (`make up-docker` / `docker compose run --rm provision`). Manifest `env:` → service `environment:` (values pass through verbatim, so `${VAR}` is interpolated from the root `.env` by compose — keep server secrets there, not in the committed manifest); `env_file:` → service `env_file:` (paths relative to `servers/<id>/`, must stay inside the repo). `stdio` `env:` is passed to the subprocess literally (no interpolation, no compose).
...
- `server.py` — the merged MCP `Server`. Every handler re-reads the request policy via `get_policy()`. **Admin lockdown:** non-admin keys see only the meta-tools (`find_tools`, `run_tool`, `run_tools`, `describe_tool`, and `plan` when a planner is configured) plus tools discovered this session; admin keys also get the full catalog. **Meta-tools:** `run_tools` runs several discovered tools concurrently through `_dispatch_tool` (bounded by `TOOL_RAG_MAX_PARALLEL`, per-call error isolation); `find_tools` takes `include_schema` (false → name+desc shortlist, lazy mode) and `describe_tool` fetches one tool's full schema **and full description** on demand (via `retriever.get_tool()`) + registers it for strict clients; the `find_tools` shortlist advertises only a truncated *teaser* description (`TOOL_RAG_SHORTLIST_DESC_CHARS`, default 300; 0 = full) — full text stays in `describe_tool` and the embedding (presentational, no reindex); `plan` (gated on `planner`) gathers policy-scoped candidates and returns an LLM-built multi-step plan (advisory, never executed). `find_tools` is the in-band discovery entry point: calling it runs the Tool-RAG retriever **scoped to the key's allowed servers + `tool_prefixes`** (via `get_policy()`) and returns matching tools with `call_name` + `input_schema`. The raw `POST /tool-rag/retrieve` HTTP handler (`tool_rag/router.py`) is policy-scoped the same way (reads `current_policy`, narrows the body's `allowed_servers` to the key's grants, applies `tool_prefixes`); enforcement is skipped only under `TOOL_RAG_WITHOUT_AUTH=1` (no policy in context by design). The `Server` also sets the MCP `initialize` `instructions` field (only when Tool-RAG is on) telling agents to call `find_tools` first. `call_tool` still works for any allowed tool by name — including tools never listed (discovered via `find_tools`); strict clients that validate calls against the advertised list are a known limitation. The retriever is wired in via `build_gateway_server(registry, retriever=...)`.
...
- `embedder.py` — `create_embedder()` factory: `LocalEmbedder` (sentence-transformers all-MiniLM-L6-v2, 384-dim, lazy-loaded) or `ApiEmbedder` (OpenAI/Ollama-shaped HTTP), selected by `TOOL_RAG_EMBEDDER=local|url` (`api` = legacy alias for `url`). `ApiEmbedder` **L2-normalizes** returned vectors (remote APIs like Ollama return unnormalized vectors; the index is `IndexFlatIP`, so normalization recovers cosine == semantic_score ∈ [0,1]); its dimension comes from `TOOL_RAG_EMBED_DIM` else a one-shot probe. Changing the embedder forces a clean index rebuild regardless of reindex mode — `indexer._load()` rejects a persisted index whose `dim` **or** `embedder` (the `model_id`: `local:<name>` / `url:<url>#<model>`) ≠ the current embedder's. This catches same-dimension model swaps that the dim check alone would miss (stale vectors under `incremental`/`off`).
- `planner.py` — optional `create_planner()` factory (`TOOL_RAG_PLANNER=off|llm`, default off): `LlmPlanner` calls an OpenAI-shaped chat-completions endpoint (`TOOL_RAG_PLANNER_URL`/`_MODEL`/`_API_KEY`, e.g. your Ollama) to turn a query + candidate tools into a validated JSON plan (`steps`/`depends_on`/`group`/`arguments_hint`); unknown `call_name`s are dropped into `missing`. Backs the `plan` meta-tool.
- `reranker.py` — optional `create_reranker()` factory (`TOOL_RAG_RERANKER=off|local`, default off): `LocalReranker` wraps a sentence-transformers `CrossEncoder` (default `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`, small + multilingual). When present, the retriever widens FAISS recall, then scores each `(query, tool_name+description)` pair, sigmoid-squashes to [0,1], and feeds that as the `semantic_score` — bounded to `MAX_RERANK_CANDIDATES=50` pairs/query.
...
- `retriever.py` — pipeline: embed query → FAISS top-K×2 (widened to ≥50 when a reranker is configured) → metadata filters (allowed_servers / permission_scope / tool_type / status=active / **server liveness**) → optional cross-encoder rerank → rank → top-K. Falls back to keyword-only DB scan if the index is empty (`fallback_used: true`); the liveness filter applies on both paths.
